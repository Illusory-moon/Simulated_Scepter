...
支持货币战争投资策略自动收集，并适配 4.4 版本的命运圣杯祈愿试炼。

- 优先选择带未解锁图鉴图标的投资策略。
- 正确处理黄金投资、白银投资产生的立即与延迟投资奖励。
- 可在 GUI 中选择完成第 1、2 或 3 位面后退出。
- 自动记录对局次数、本局解锁的投资策略和用时。
...
每局完成后，统计会追加到 `config/backup/currency_count.txt`。铁血战士和货币战争会分别保存统计，但不要同时运行两个脚本控制同一个游戏窗口。
