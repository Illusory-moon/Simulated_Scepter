...
def match_multiple_targets(processed_image,mode=1, threshold=0.5, max_per_template=10):
    ...
    if mode == 1:
        kind_list=['event', 'wait', 'trade', 'adventure', 'reward', 'battle', 'elite', 'bugevent', 'bugbattle', 'head', 'boss']
    else:
        kind_list=['event', 'wait', 'trade','trade2', 'adventure', 'reward', 'reward2','battle', 'elite', 'bugevent', 'bugbattle', 'head', 'boss',
         'blank']
    for name in kind_list:
        ...
        if xs.size == 0:
            continue
        # 候选按得分降序
        candidates = sorted(((float(res[y, x]), int(x), int(y)) for y, x in zip(ys, xs)), key=lambda t: t[0], reverse=True)
        accepted_centers = []
        added = 0
...
def compute_start_point_from_crop(image, crop_coords=(50, 63, 92, 104)):
    ...
    grey=cv2.bitwise_and(search_gray, search_gray, mask=find_image_in_folder(f'gray_image/', 'head_mask'))
    res = cv2.matchTemplate(grey, tpl_gray, cv2.TM_CCOEFF_NORMED)
...
if __name__ == '__main__':
    ...
    if start is None:
        start = compute_start_point_from_crop(test_image,[1003,929,1035,965])
    ...
    print('\n起点索引 =', start_idx, '名称 =', next((n['name'] for n in nodes if n['idx']==start_idx), None))

    # 打印边的连接情况
    # print('\n边的连接情况:')
    # for src_idx, targets in edges.items():
    #     if targets:
    #         src_name = nodes[src_idx]['name']
    #         tgt_names = [f"{t}({nodes[t]['name']})" for t in targets]
    #         print(f"  {src_idx}({src_name}) -> {tgt_names}")

    path, total_weight, end_idx = max_weight_path(nodes, edges, start_idx)
    if path:
        weight_ranges = {
        ...
        print(f'\n路径理论期望值：total_weight={total_weight:.3f}')
    ...
    print(f'\n路径结果：path={len(path) if path else 0}, total_weight={total_weight}, end_idx={end_idx}')
    print('\n找到的路径 (索引 -> 名称，权重):')
    for n in path:
        print(f"  {n['idx']} -> {n['name']} (w={n['weight']})")
        
    # 评估最佳单节点替换（使用期权价值模型，t=0.2）
    # 先计算所有节点到起点的距离
    steps_cache = compute_all_max_steps(nodes, edges, start_idx)
    
    # 打印距离起点为 1 的所有点位
    distance_1_nodes = [idx for idx, dist in steps_cache.items() if dist == 1]
    print(f'\n距离起点为 1 的点位索引：{distance_1_nodes}')
    if distance_1_nodes:
        print('详细信息：')
        for idx in distance_1_nodes:
            node = next(n for n in nodes if n['idx'] == idx)
            print(f'  索引 {idx}: 类型={node["name"]}, 中心=({node["cx"]:.1f},{node["cy"]:.1f})')
    
    best_path, best_weight, best_end_idx, best_replace_idx, delta, discounted_delta = evaluate_best_single_replacement(nodes, edges, start_idx, t=0.2)
    if best_replace_idx is None or discounted_delta <= 0:
        ...
        print(f'替换后路径总权重：{best_weight:.3f} (原权重：{total_weight:.3f})')
        highlight = b
        alt_path = best_path  # 获取替换后的新路径
        # 打印新旧路径对比
    ...
    try:
        display_matches(test_image, matches, path=path, highlight_idx=highlight, save_path='matches_preview.png', wait_ms=0, font_size_override=14, alt_path=alt_path)
    except Exception as e:
        print('显示匹配结果时出错:', e)
