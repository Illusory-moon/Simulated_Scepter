...
class ExactCountdownDP:
    ...
    def _offer(self, pending: Optional[dict], plan: _Plan, action: object,
        ...
        if pending is not None:
            ...
            if same and (same.state.countdown > countdown or (
                    same.state.countdown == countdown
    ...
    def _collect(self, pending: dict, plan: _Plan) -> None:
        """先按精确感染 mask 去重；同一节点到齐后再统一做支配剪枝。"""
    ...
    def _pareto_frontier(self, plans: dict) -> dict:
        ...
        ordered = sorted(
            plans.values(),
            key=lambda plan: (
                -plan.state.countdown, -plan.state.infected.bit_count(),
